...
static void sign_challenge(const char* challenge, const sso_context_t* ctx, char* out_sig) {
    ...
    HMAC(EVP_sha256(), cfg->token_secret, strlen(cfg->token_secret), (const unsigned char*)challenge, strlen(challenge), mac, &len);
    for (int i = 0; i < 32; i++) {
...
sso_error_t handle_webauthn_register(sso_context_t* ctx, const http_request_t* req, http_response_t* resp) {
    ...
    char* cred_id = NULL;
    char* pub_key = NULL;
    sso_error_t err = webauthn_register_verify(attestation_b64u, client_data_b64u, challenge, expected_origin, &cred_id, &pub_key);
    yyjson_doc_free(req_doc);
    ...
    if (user_get_by_id(umgr, actx->user.id, &user) == SSO_OK) {
        ...
        if (!mroot) { mroot = yyjson_mut_obj(mdoc); yyjson_mut_doc_set_root(mdoc, mroot); }
...
sso_error_t handle_webauthn_login_challenge(sso_context_t* ctx, const http_request_t* req, http_response_t* resp) {
    ...
    if (!cred_id) {
        if (attr_doc) yyjson_doc_free(attr_doc);
...
sso_error_t handle_webauthn_login(sso_context_t* ctx, const http_request_t* req, http_response_t* resp) {
    ...
    sso_error_t err = webauthn_login_verify(auth_data_b64u, client_data_b64u, signature_b64u, challenge, expected_origin, pub_key);
    yyjson_doc_free(req_doc);
    if (attr_doc) yyjson_doc_free(attr_doc);
