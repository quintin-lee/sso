...
static size_t b64u_encode(const unsigned char* in, size_t in_len, char* out) {
    ...
    for (size_t i = 0; i < in_len; i += 3) {
        ...
        if (i + 1 < in_len) out[out_len++] = basis64[((b2 & 0x0f) << 2) | (b3 >> 6)];
        if (i + 2 < in_len) out[out_len++] = basis64[b3 & 0x3f];
    }
    out[out_len] = '\0';
    return out_len;
}
...
static size_t b64u_decode(const char* in, unsigned char* out) {
    ...
    if (char_count == 2) out[out_len++] = (bits >> 4) & 0xff;
...
char* webauthn_generate_challenge(void) {
    ...
    char* b64 = malloc(64);
    if (b64) b64u_encode(buf, sizeof(buf), b64);
    return b64;
}
...
static const unsigned char* find_auth_data(const unsigned char* cbor, size_t len, size_t* out_len) {
    ...
    for (size_t i = 0; i < len - sizeof(authData_key); i++) {
        if (memcmp(cbor + i, authData_key, sizeof(authData_key)) == 0) {
            ...
            p++;
            size_t data_len = 0;
            if (val < 24) { data_len = val; }
            else if (val == 24 && p < len) { data_len = cbor[p++]; }
            else if (val == 25 && p + 1 < len) { data_len = (cbor[p] << 8) | cbor[p+1]; p += 2; }
            else return NULL;
...
static sso_error_t extract_cose_ec2_key(const unsigned char* cose_cbor, size_t len, char** out_pem) {
    ...
    if (!x || !y) return SSO_ERR_TOKEN_INVALID;
...
static sso_error_t verify_client_data(const unsigned char* client_data_raw, size_t len, const char* expected_challenge, const char* expected_origin) {
    yyjson_doc* doc = yyjson_read((const char*)client_data_raw, len, 0);
    if (!doc) return SSO_ERR_INVALID_PARAM;
...
sso_error_t webauthn_register_verify(
    const char* attestation_b64u, 
    const char* client_data_b64u, 
    const char* expected_challenge, 
    const char* expected_origin,
    char** out_credential_id_b64u,
    char** out_public_key_pem) 
{
    ...
    if (err != SSO_OK) goto end;

    size_t auth_data_len = 0;
    const unsigned char* auth_data = find_auth_data(att, att_len, &auth_data_len);
    if (!auth_data || auth_data_len < 37) { err = SSO_ERR_INVALID_PARAM; goto end; }

    /* Parse authData */
    unsigned char flags = auth_data[32];
    if (!(flags & 0x40)) { err = SSO_ERR_INVALID_PARAM; goto end; } /* Attested credential data flag must be set */
    
    size_t p = 37; /* RPID Hash (32) + Flags (1) + SignCount (4) */
    p += 16; /* AAGUID */
    if (p + 2 > auth_data_len) { err = SSO_ERR_INVALID_PARAM; goto end; }
    
    int cred_len = (auth_data[p] << 8) | auth_data[p+1];
    p += 2;
    if (p + cred_len > auth_data_len) { err = SSO_ERR_INVALID_PARAM; goto end; }
...
sso_error_t webauthn_login_verify(
    const char* authenticator_data_b64u,
    const char* client_data_b64u,
    ...
    const char* public_key_pem)
{
    ...
    if (!pkey) { err = SSO_ERR_GENERAL; free(signed_data); goto end; }
