...
sso_error_t handle_login(sso_context_t* ctx, const http_request_t* req, http_response_t* resp) {
	...
	snprintf(buf, 8192,
			 ...
			 "\"display_name\":\"%s\""
			 "}",
			 (long long)access_token.expires_at, (unsigned long long)user.id, user.username, user.display_name);
...
sso_error_t handle_me(sso_context_t* ctx, const http_request_t* req, http_response_t* resp) {
	...
	snprintf(buf, sizeof(buf),
			 ...
			 "\"token_jti\":\"%s\""
			 "}",
			 (unsigned long long)auth->user.id, auth->user.username, auth->user.email, auth->user.display_name,
			 auth->token.jti);
	sso_response_ok(resp, buf);
	return SSO_OK;
}
...
sso_error_t handle_session_check(sso_context_t* ctx, const http_request_t* req, http_response_t* resp) {
	...
	snprintf(resp->extra_headers, sizeof(resp->extra_headers),
			 ...
			 "X-SSO-Email: %s\r\n"
			 "X-SSO-User-Id: %llu\r\n",
			 user.username, user.email, (unsigned long long)user.id);
	...
	snprintf(buf, sizeof(buf),
			 ...
			 "\"email\":\"%s\""
			 "}",
			 (unsigned long long)user.id, user.username, user.email);
	sso_response_ok(resp, buf);
	token_destroy(&tok);
	return SSO_OK;
}
