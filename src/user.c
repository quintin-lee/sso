 ...
 * ----------------------------------------------------------------------- */
static void hash_password(const char* password, char* out_hash, size_t out_len, unsigned long opslimit,
	...
	if (crypto_pwhash_str(out_hash, password, strlen(password), opslimit, memlimit) != 0) {
		if (out_len > 0)
			out_hash[0] = '\0';
	}
}
...
void user_manager_set_hash_params(user_manager_t* mgr, unsigned long opslimit, unsigned long memlimit) {
	if (!mgr)
		return;
 ...
 * CRUD
 * ======================================================================== */
sso_error_t user_create(user_manager_t* mgr, const char* username, const char* password, const char* email,
						const char* display_name, user_t* out) {
	if (!mgr || !username || !password)
		return SSO_ERR_INVALID_PARAM;

	storage_backend_t* sb = (storage_backend_t*)mgr->ctx->storage_backend;
	if (!sb || !sb->user_create)
	...
	if (display_name)
		sso_strlcpy(user.display_name, display_name, SSO_MAX_DISPLAY_NAME);
	...
	hash_password(password, user.password_hash, sizeof(user.password_hash), mgr->hash_opslimit, mgr->hash_memlimit);
...
sso_error_t user_list(user_manager_t* mgr, const char* q, int status, int offset, int limit, sso_id_t* ids,
					  size_t* count, size_t* total_count) {
	if (!mgr || !ids || !count || !total_count)
		return SSO_ERR_INVALID_PARAM;
	storage_backend_t* sb = (storage_backend_t*)mgr->ctx->storage_backend;
	if (!sb || !sb->user_list)
		return SSO_ERR_NOT_IMPLEMENTED;
 ...
 * Authentication
 * ======================================================================== */
sso_error_t user_authenticate(user_manager_t* mgr, const char* username, const char* password, user_t* out) {
	if (!mgr || !username || !password || !out)
		return SSO_ERR_INVALID_PARAM;
	...
	if (stored.status != USER_STATUS_ACTIVE)
		return SSO_ERR_AUTH_FAILED;

	/* Verify password against stored argon2id hash */
	if (crypto_pwhash_str_verify(stored.password_hash, password, strlen(password)) != 0) {
		return SSO_ERR_AUTH_FAILED;
	}

	/* Check password expiry */
	sso_config_t* cfg = (sso_config_t*)sso_get_config(mgr->ctx);
	if (cfg && cfg->password_policy.expiry_days > 0 && stored.password_set_at > 0) {
		sso_timestamp_t now		  = sso_timestamp_now();
		sso_timestamp_t expiry_ms = (sso_timestamp_t)cfg->password_policy.expiry_days * 86400000LL;
		if (stored.password_set_at + expiry_ms < now) {
			return SSO_ERR_PASSWORD_EXPIRED;
		}
	}
...
sso_error_t user_set_password(user_manager_t* mgr, sso_id_t user_id, const char* new_password) {
	if (!mgr || !new_password)
		return SSO_ERR_INVALID_PARAM;
	...
	hash_password(new_password, user.password_hash, sizeof(user.password_hash), mgr->hash_opslimit, mgr->hash_memlimit);
	user.password_set_at = sso_timestamp_now();

	return user_update(mgr, &user);
}

sso_error_t user_set_status(user_manager_t* mgr, sso_id_t user_id, user_status_t status) {
	if (!mgr)
		return SSO_ERR_INVALID_PARAM;
 ...
 * Role & Group queries
 * ======================================================================== */
sso_error_t user_get_roles(user_manager_t* mgr, sso_id_t user_id, sso_id_t* role_ids, size_t* count, size_t max) {
	if (!mgr || !role_ids || !count)
		return SSO_ERR_INVALID_PARAM;
	storage_backend_t* sb = (storage_backend_t*)mgr->ctx->storage_backend;
	if (!sb || !sb->get_user_roles)
		return SSO_ERR_NOT_IMPLEMENTED;
	return sb->get_user_roles(sb, user_id, role_ids, count, max);
}

sso_error_t user_get_groups(user_manager_t* mgr, sso_id_t user_id, sso_id_t* group_ids, size_t* count, size_t max) {
	if (!mgr || !group_ids || !count)
		return SSO_ERR_INVALID_PARAM;
	storage_backend_t* sb = (storage_backend_t*)mgr->ctx->storage_backend;
	if (!sb || !sb->get_user_groups)
		return SSO_ERR_NOT_IMPLEMENTED;
