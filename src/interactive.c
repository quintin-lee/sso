/*
 * interactive.c — Interactive text-based configuration shell.
 *
 * Provides a console menu-driven interface for creating and managing
 * SSO entities (users, roles, groups, policies) across all seven
 * permission strategy types.  Useful for offline administration and
 * policy prototyping without running the HTTP server.
 */
...
static void config_functional(policy_manager_t* pmgr) {
	...
	snprintf(rules, sizeof(rules), "{\"functions\":[{\"code\":\"%s\",\"effect\":\"%s\"}]}", code, effect);

	policy_t	p;
	sso_error_t err = policy_create(pmgr, name, PERM_STRATEGY_FUNCTIONAL, POLICY_EFFECT_ALLOW, 50, rules, &p);
	printf("  → %s (id=%lu)\n", err == SSO_OK ? "Policy created" : sso_strerror(err), (unsigned long)p.id);
}
...
static void config_api(policy_manager_t* pmgr) {
	...
	char rules[SSO_MAX_RULES_JSON];
	snprintf(rules, sizeof(rules), "{\"endpoints\":[{\"method\":\"%s\",\"path\":\"%s\",\"effect\":\"%s\"}]}", method,
			 path, effect);

	policy_t	p;
	sso_error_t err = policy_create(pmgr, name, PERM_STRATEGY_API, POLICY_EFFECT_ALLOW, 50, rules, &p);
	printf("  → %s (id=%lu)\n", err == SSO_OK ? "Policy created" : sso_strerror(err), (unsigned long)p.id);
}
...
static void config_data(policy_manager_t* pmgr) {
	...
	while (tok) {
		...
		while (endp > tok && *endp == ' ')
			*endp-- = '\0';
		if (flen > 0) {
			strncat(field_list, ",", sizeof(field_list) - flen - 1);
			flen++;
		}
		char entry[64];
	...
	snprintf(rules, sizeof(rules),
			 "{\"rules\":[{\"resource\":\"%s\",\"scope\":\"%s\",\"fields\":[%s]}],\"effect\":\"%s\"}", resource, scope,
			 field_list, effect);

	policy_t	p;
	sso_error_t err = policy_create(pmgr, name, PERM_STRATEGY_DATA, POLICY_EFFECT_ALLOW, 50, rules, &p);
	printf("  → %s (id=%lu)\n", err == SSO_OK ? "Policy created" : sso_strerror(err), (unsigned long)p.id);
}
...
static void config_rbac(policy_manager_t* pmgr) {
	...
	char rules[SSO_MAX_RULES_JSON];
	snprintf(rules, sizeof(rules), "{\"roles\":[{\"name\":\"%s\",\"effect\":\"%s\"}]}", role_name, effect);

	policy_t	p;
	sso_error_t err = policy_create(pmgr, name, PERM_STRATEGY_RBAC, POLICY_EFFECT_ALLOW, 50, rules, &p);
	printf("  → %s (id=%lu)\n", err == SSO_OK ? "Policy created" : sso_strerror(err), (unsigned long)p.id);
}
...
static void config_location(policy_manager_t* pmgr) {
	...
	char rules[SSO_MAX_RULES_JSON];
	snprintf(rules, sizeof(rules), "{\"locations\":[{\"type\":\"ip_cidr\",\"value\":\"%s\",\"effect\":\"%s\"}]}", cidr,
			 effect);

	policy_t	p;
	sso_error_t err = policy_create(pmgr, name, PERM_STRATEGY_LOCATION, POLICY_EFFECT_ALLOW, 50, rules, &p);
	printf("  → %s (id=%lu)\n", err == SSO_OK ? "Policy created" : sso_strerror(err), (unsigned long)p.id);
}
...
static void config_lbac(policy_manager_t* pmgr) {
	...
	char rules[SSO_MAX_RULES_JSON];
	snprintf(rules, sizeof(rules), "{\"labels\":[{\"name\":\"%s\",\"effect\":\"%s\"}]}", label, effect);

	policy_t	p;
	sso_error_t err = policy_create(pmgr, name, PERM_STRATEGY_LBAC, POLICY_EFFECT_ALLOW, 55, rules, &p);
	printf("  → %s (id=%lu)\n", err == SSO_OK ? "Policy created" : sso_strerror(err), (unsigned long)p.id);
}
...
static void config_abac(policy_manager_t* pmgr) {
	...
	char rules[SSO_MAX_RULES_JSON];
	snprintf(rules, sizeof(rules),
			 "{\"conditions\":[{\"source\":\"%s\",\"attr\":\"%s\",\"op\":\"%s\",\"value\":\"%s\"}],\"logic\":\"%s\","
			 "\"effect\":\"%s\"}",
			 source, attr, op, value, logic, effect);

	policy_t	p;
	sso_error_t err = policy_create(pmgr, name, PERM_STRATEGY_ABAC, POLICY_EFFECT_ALLOW, 50, rules, &p);
	printf("  → %s (id=%lu)\n", err == SSO_OK ? "Policy created" : sso_strerror(err), (unsigned long)p.id);
}
...
static void action_check(sso_context_t* ctx) {
	...
	switch (st) {
		case 1: { /* Functional */
			...
			prompt_line("  Function code (e.g. report:view): ", code, sizeof(code));
			if (code[0])
				err = perm_check_function(ctx, uid, code, &allowed);
		...
		case 2: { /* API */
			...
			if (method[0] && path[0])
				err = perm_check_api(ctx, uid, method, path, &allowed);
		...
		case 3: { /* Data */
			...
			if (rtype[0]) {
				char** fields = NULL;
				size_t fcount = 0;
				err			  = perm_check_data(ctx, uid, rtype, record[0] ? record : NULL, &allowed, &fields, &fcount);
		...
		case 4: { /* RBAC */
			...
			if (rname[0])
				err = perm_check_rbac(ctx, uid, rname, &allowed);
		...
		case 5: { /* Location */
			...
			if (ip[0])
				err = perm_check_location(ctx, uid, ip, NULL, &allowed);
		...
		case 6: { /* ABAC */
			...
			if (attrs[0])
				err = perm_check_abac(ctx, uid, attrs, NULL, NULL, &allowed);
			printf("  check_abac() → %s\n", allowed ? "ALLOW" : "DENY");
			break;
		}
		case 7: { /* LBAC (Label-Based) */
			...
			if (u_labels[0] && r_label[0])
				err = perm_check_lbac(ctx, uid, u_labels, r_label, &allowed);
...
static void action_list(policy_manager_t* pmgr) {
	...
	for (sso_id_t i = 1; i <= 64; i++) {
		...
		if (policy_get_by_id(pmgr, i, &p) == SSO_OK) {
			printf("  [%2lu] %-30s type=%-2d pri=%-3d status=%s  rules=%.40s\n", (unsigned long)p.id, p.name,
				   p.strategy_type, p.priority, p.status == POLICY_STATUS_ENABLED ? "enabled" : "disabled", p.rules);
		}
	}
}
...
static void action_create_user(user_manager_t* umgr) {
	...
	printf("  → %s (id=%lu)\n", err == SSO_OK ? "Created" : sso_strerror(err), (unsigned long)u.id);
}
...
static void action_list_users(sso_context_t* ctx) {
	...
	for (size_t i = 0; i < count; i++) {
		user_t u;
		if (user_get_by_id(umgr, ids[i], &u) != SSO_OK)
			continue;
		...
		printf("  [%2lu] %-12s  status=%-6s  email=%-20s", (unsigned long)u.id, u.username,
			   u.status == USER_STATUS_ACTIVE	? "active"
			   : u.status == USER_STATUS_LOCKED ? "locked"
												: "inactive",
			   u.email);
...
static void action_create_role(role_manager_t* rmgr) {
	...
	printf("  → %s (id=%lu)\n", err == SSO_OK ? "Created" : sso_strerror(err), (unsigned long)r.id);
}
...
static void action_list_roles(role_manager_t* rmgr) {
	...
	for (size_t i = 0; i < count; i++) {
		role_t r;
		if (role_get_by_id(rmgr, ids[i], &r) != SSO_OK)
			continue;
		printf("  [%2lu] %-16s  parent=%-3lu  desc=%s\n", (unsigned long)r.id, r.name, (unsigned long)r.parent_role_id,
			   r.description);
	}
...
static void action_create_group(group_manager_t* gmgr) {
	...
	printf("  → %s (id=%lu)\n", err == SSO_OK ? "Created" : sso_strerror(err), (unsigned long)g.id);
}
...
static void action_list_groups(group_manager_t* gmgr) {
	...
	for (size_t i = 0; i < count; i++) {
		group_t g;
		if (group_get_by_id(gmgr, ids[i], &g) != SSO_OK)
			continue;
		printf("  [%2lu] %-16s  parent=%-3lu  desc=%s\n", (unsigned long)g.id, g.name, (unsigned long)g.parent_group_id,
			   g.description);
	}
	...
	while (running) {
		...
		switch (opt) {
			...
			case 10:
				...
				break;
			case 11:
				action_create_user(umgr);
				break;
			case 12:
				action_list_users(&ctx);
				break;
			case 13:
				action_delete_user(umgr);
				break;
			case 14:
				action_create_role(rmgr);
				break;
			case 15:
				action_list_roles(rmgr);
				break;
			case 16:
				action_delete_role(rmgr);
				break;
			case 17:
				action_assign_role_to_user(rmgr);
				break;
			case 18:
				action_unassign_role_from_user(rmgr);
				break;
			case 19:
				action_create_group(gmgr);
				break;
			case 20:
				action_list_groups(gmgr);
				break;
			case 21:
				action_add_user_to_group(gmgr);
				break;
			case 22:
				action_remove_user_from_group(gmgr);
				break;
			case 0:
				running = 0;
				break;
