/*
 * mfa.c — TOTP multi-factor authentication (RFC 6238) implementation.
 *
 * Generates and validates time-based one-time passwords using HMAC-SHA1.
 * Provides base32-encoded secret generation, provisioning URI construction
 * (for QR code enrollment), and TOTP code verification with a configurable
 * time-step window to accommodate clock skew.
 */
...
bool mfa_verify_totp(const char* base32_secret, const char* code) {
	if (!base32_secret || !code || strlen(code) != 6)
		return false;
	uint8_t key[64];
