...
static sso_error_t bootstrap_data(sso_context_t* ctx) {
	...
	if (!admin_password || admin_password[0] == '\0') {
		...
		snprintf(random_pass, sizeof(random_pass), "admin-%02x%02x%02x%02x", rand_bytes[0], rand_bytes[1],
...
static int run_server(sso_config_t* cfg, const char* config_path) {
	...
	route_t routes[] = {
			...
			{"/api/v1/auth/rotate-keys", HTTP_POST, handle_rotate_keys, true},
...
int main(int argc, char** argv) {
	...
	} else if (argc > 1 && strcmp(argv[argc - 1], "--interactive") == 0) {
