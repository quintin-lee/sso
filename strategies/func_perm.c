 ...
 *
 * Controls access to application features (functions, menus, buttons)
 * by matching a function_code string against allow/deny rule lists.
 * Uses wildcard matching (*) for prefix/suffix/glob patterns so that
 * a single rule like "user:*" covers all user-related functions.
 *
 * At compile time the JSON rules are parsed into a flat array of
 * func_rule_item_t structs.  Evaluation walks the array in order and
 * returns the first matching allow or deny decision.
 *
 * JSON rule format:
 *   { "allow": ["user:create", "user:read", "user:*"],
 *     "deny":  ["user:delete"] }
 *
 * Example: a user with the above rules can create and read any user
 * record but is explicitly denied the "user:delete" function.
 */
...
static sso_error_t func_compile(permission_strategy_t* self, const char* rules_json, void** compiled_rule) {
	(void)self;
...
static void func_free_compiled(permission_strategy_t* self, void* compiled_rule) {
	(void)self;
	if (!compiled_rule)
		return;
...
static sso_error_t func_evaluate(permission_strategy_t* self, eval_context_t* ctx, const policy_t* policy,
...
static sso_error_t func_validate(permission_strategy_t* self, const char* rules_json) {
	(void)self;
	if (!rules_json)
		return SSO_ERR_INVALID_PARAM;
	yyjson_doc* doc = yyjson_read(rules_json, strlen(rules_json), 0);
