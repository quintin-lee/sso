/*
 * abac_perm.c — Attribute-Based Access Control (ABAC) permission strategy.
 *
 * Evaluates boolean expressions over subject, resource, and environment
 * attributes.  Each condition specifies a source (subject/resource/
 * environment), an attribute name, a comparison operator, and an expected
 * value.  Supported operators: eq, neq, gt, gte, lt, lte, contains, in.
 * Conditions can be combined with AND (default) or OR logic.
 *
 * At compile time the JSON rules are parsed into abac_condition_t arrays.
 * Evaluation parses the runtime attribute JSON documents and applies
 * each operator against the corresponding attribute value.
 *
 * JSON rule format:
 *   { "conditions": [
 *       {"source": "subject", "attr": "dept", "op": "eq", "value": "eng"}
 *     ],
 *     "logic": "and",
 *     "effect": "allow" }
 *
 * Example: allows access only when the requesting subject's department
 * attribute equals "eng".
 */
...
typedef enum { ABAC_SRC_SUBJECT, ABAC_SRC_RESOURCE, ABAC_SRC_ENVIRONMENT } abac_source_t;
...
static sso_error_t abac_compile(permission_strategy_t* self, const char* rules_json, void** compiled_rule) {
	(void)self;
	...
	yyjson_arr_foreach(conditions, idx, max, item) {
		...
		if (yyjson_is_str(src)) {
			...
			if (strcmp(src_str, "resource") == 0)
				compiled->conditions[idx].source = ABAC_SRC_RESOURCE;
			else if (strcmp(src_str, "environment") == 0)
				compiled->conditions[idx].source = ABAC_SRC_ENVIRONMENT;
			else
...
static void abac_free_compiled(permission_strategy_t* self, void* compiled_rule) {
	(void)self;
	if (!compiled_rule)
		return;
...
static sso_error_t abac_evaluate(permission_strategy_t* self, eval_context_t* ctx, const policy_t* policy,
	...
	if (ctx->params.abac.subject_attrs[0] != '\0') {
		...
		if (subject_doc)
			subject_root = yyjson_doc_get_root(subject_doc);
	}
	if (ctx->params.abac.resource_attrs[0] != '\0') {
		...
		if (resource_doc)
			resource_root = yyjson_doc_get_root(resource_doc);
	}
	if (ctx->environment[0] != '\0') {
		env_doc = yyjson_read(ctx->environment, strlen(ctx->environment), 0);
		if (env_doc)
			env_root = yyjson_doc_get_root(env_doc);
	}
	...
	for (size_t i = 0; i < compiled->count; i++) {
		...
		switch (cond->source) {
			case ABAC_SRC_RESOURCE:
				source_root = resource_root;
				break;
			case ABAC_SRC_ENVIRONMENT:
	...
	if (subject_doc)
		yyjson_doc_free(subject_doc);
	if (resource_doc)
		yyjson_doc_free(resource_doc);
	if (env_doc)
...
static sso_error_t abac_validate(permission_strategy_t* self, const char* rules_json) {
	(void)self;
	if (!rules_json)
		return SSO_ERR_INVALID_PARAM;
	yyjson_doc* doc = yyjson_read(rules_json, strlen(rules_json), 0);
