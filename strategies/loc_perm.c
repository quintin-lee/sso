 ...
 *
 * Controls access based on the client's source IP address or geographic
 * country code.  Supports three match types: IP CIDR notation for
 * subnet matching (e.g. 10.0.0.0/8), exact IP address matching, and
 * ISO 3166-1 alpha-2 country code matching.
 *
 * At compile time the JSON rules are parsed into loc_rule_item_t
 * entries with pre-computed CIDR network/prefix values so that
 * evaluation never re-parses the rule text.
 *
 * JSON rule format:
 *   { "allowed_ips":       ["10.0.0.0/8", "192.168.1.0/24"],
 *     "allowed_countries": ["US", "CA"] }
 *
 * Example: grants access only to clients on the corporate 10.x subnet
 * or originating from US/CA geo locations.
 */
...
/* Types */
/* ----------------------------------------------------------------------- */
typedef enum { LOC_TYPE_IP_CIDR, LOC_TYPE_GEO, LOC_TYPE_EXACT } loc_rule_type_t;
...
static bool ip_matches_cidr(const char* ip_str, const char* cidr_str, unsigned int cidr_network,
							unsigned int cidr_prefix) {
	...
	if (cidr_prefix > 0) {
		unsigned int mask = (cidr_prefix >= 32) ? 0xFFFFFFFFU : (0xFFFFFFFFU << (32 - cidr_prefix));
...
static sso_error_t loc_compile(permission_strategy_t* self, const char* rules_json, void** compiled_rule) {
	(void)self;
	...
	compiled->items = (loc_rule_item_t*)calloc(count, sizeof(loc_rule_item_t));
	if (!compiled->items) {
		free(compiled);
		yyjson_doc_free(doc);
		return SSO_ERR_OUT_OF_MEMORY;
	...
	yyjson_arr_foreach(location_list, idx, max, item) {
		if (yyjson_is_str(item)) {
			...
			sso_strlcpy(compiled->items[idx].value, yyjson_get_str(item), 255);
			...
			if (yyjson_is_str(value))
				sso_strlcpy(compiled->items[idx].value, yyjson_get_str(value), 255);

			if (yyjson_is_str(type) && strcmp(yyjson_get_str(type), "geo") == 0) {
				...
				if (!parse_cidr(compiled->items[idx].value, &compiled->items[idx].cidr_network,
...
static void loc_free_compiled(permission_strategy_t* self, void* compiled_rule) {
	(void)self;
	if (!compiled_rule)
		return;
...
static sso_error_t loc_evaluate(permission_strategy_t* self, eval_context_t* ctx, const policy_t* policy,
	...
	for (size_t i = 0; i < compiled->count; i++) {
		...
		switch (compiled->items[i].type) {
			case LOC_TYPE_IP_CIDR:
				matched = ip_matches_cidr(ip, compiled->items[i].value, compiled->items[i].cidr_network,
										  compiled->items[i].cidr_prefix);
				break;

			case LOC_TYPE_EXACT:
				matched = (strcmp(ip, compiled->items[i].value) == 0);
				break;

			case LOC_TYPE_GEO: {
				const char* country = ctx->params.location.geo_country;
				if (country && country[0] != '\0')
					matched = (strcmp(country, compiled->items[i].value) == 0);
				break;
			}
		}
...
static sso_error_t loc_validate(permission_strategy_t* self, const char* rules_json) {
	(void)self;
	if (!rules_json)
		return SSO_ERR_INVALID_PARAM;
	yyjson_doc* doc = yyjson_read(rules_json, strlen(rules_json), 0);
