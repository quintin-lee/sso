<template>
  <div class="bg-[var(--bg-card)] rounded-2xl border border-[var(--border-primary)] overflow-hidden">
    <DataTable :value="displayedLogs" paginator :rows="20" :loading="loading">
      <!-- Time Column -->
      <Column field="timestamp_ms" :header="$t('sidebar.logs')" class="w-44">
      ...
      <!-- Actor Column -->
      <Column field="username" header="Actor" class="w-28">
        <template #body="slotProps">
      ...
      <Column field="action" header="Type" class="w-24">
        <template #body="slotProps">
          <span v-if="slotProps.data.action === 'admin'" class="px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider bg-amber-500/10 text-amber-400">Admin</span>
          <span v-else class="px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider bg-indigo-500/10 text-indigo-400">Check</span>
        </template>
      </Column>

      <!-- Operation (admin) / Decision (check) Column -->
      <Column field="operation" header="Operation" class="w-36">
        <template #body="slotProps">
      ...
      <Column field="resource" header="Resource" class="w-28">
        <template #body="slotProps">
      ...
      <Column field="details" header="Details" class="font-mono text-xs text-[var(--text-muted)] max-w-xs">
        <template #body="slotProps">
      ...
      <Column field="ip_address" header="IP" class="w-28 hidden lg:table-cell">
        <template #body="slotProps">
      ...
      <Column field="duration_ms" header="Latency" class="w-20">
        <template #body="slotProps">
      ...
      <template #empty>
        <div class="flex flex-col items-center justify-center py-16 text-[var(--text-muted)]">
          ...
          <p class="text-xs mt-1 opacity-60">Logs will appear when events are recorded</p>
        </div>
      </template>
    </DataTable>
  </div>
