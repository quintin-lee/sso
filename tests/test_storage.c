/*
 * test_storage.c — Unit tests for storage backends (SQLite, memory).
 *
 * Exercises the storage_backend vtable: user/role/group/policy CRUD,
 * recursive hierarchy queries, token persistence, edge cases with
 * missing records, and concurrent access patterns.
 */
...
#define mu_run_test_arg(test, arg)                                                                                     \
	do {                                                                                                               \
		const char* message = test(arg);                                                                               \
		tests_run++;                                                                                                   \
		if (message)                                                                                                   \
			return message;                                                                                            \
	} while (0)
...
static const char* test_generic_assignments(storage_backend_t* s) {
	...
	user_t u;
	fill_user(&u, "assignuser");
	ASSERT_INT_EQUAL(s->user_create(s, &u), SSO_OK);
	role_t r;
	fill_role(&r, "assignrole");
	ASSERT_INT_EQUAL(s->role_create(s, &r), SSO_OK);
	group_t g;
	fill_group(&g, "assigngroup");
	ASSERT_INT_EQUAL(s->group_create(s, &g), SSO_OK);
...
static const char* test_generic_duplicate(storage_backend_t* s) {
	...
	user_t u1;
	fill_user(&u1, "dupname");
	s->user_create(s, &u1);
	user_t u2;
	fill_user(&u2, "dupname");
	ASSERT_INT_EQUAL(s->user_create(s, &u2), SSO_ERR_ALREADY_EXISTS);

	role_t r1;
	fill_role(&r1, "duprole");
	s->role_create(s, &r1);
	role_t r2;
	fill_role(&r2, "duprole");
