/*
 * test_config.c — Unit tests for TOML configuration loading and env-override.
 *
 * Verifies config_init() parses valid config, handles partial/missing
 * TOML files, and correctly applies environment variable overrides.
 */
...
static const char* test_config_load_fallback() {
	...
	if (!fp)
		return "Failed to create test_fallback.toml";
	fprintf(fp, "[database]\npath = \"legacy.db\"\n");
	fclose(fp);
