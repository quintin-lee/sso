/*
 * test_role.c — Unit tests for the role management module.
 *
 * Tests role CRUD, hierarchical nesting (parent_role_id with
 * recursive ancestor inheritance), role assignment/unassignment,
 * and effective role resolution via get_user_roles_with_ancestors().
 */
...
static void setup(storage_backend_t** storage, sso_context_t* ctx, role_manager_t** rmgr, user_manager_t** umgr) {
