/*
 * test_policy.c — Unit tests for the policy management module.
 *
 * Tests policy CRUD, JSON rule validation for all 7 strategy types,
 * policy assignment to users/roles/groups, priority ordering,
 * and the resolution flattening (user + role + group policies).
 */
...
static void setup(storage_backend_t** storage, sso_context_t* ctx, user_manager_t** umgr, role_manager_t** rmgr,
	...
	if (umgr) {
		...
		user_manager_set_hash_params(*umgr, 2, 67108864);
		ctx->user_mgr = *umgr;
	}
	if (rmgr) {
...
static void teardown(storage_backend_t* storage, user_manager_t* umgr, role_manager_t* rmgr, policy_manager_t* pmgr,
...
static const char* test_permission_rbac_basic() {
	...
	user_t user;
	user_create(umgr, "alice", "p", "a@e.c", "A", &user);
	role_t admin;
	role_create(rmgr, "admin", "Admin", SSO_ID_NONE, &admin);
...
static const char* test_permission_api_basic() {
	...
	user_t user;
	user_create(umgr, "bob", "p", "b@e.c", "B", &user);
...
static const char* test_permission_abac_basic() {
	...
	user_t user;
	user_create(umgr, "charlie", "p", "c@e.c", "C", &user);

	policy_t	pol;
	const char* rules = "{\"conditions\":[{\"source\":\"resource\",\"attr\":\"owner_id\",\"op\":\"eq\",\"value\":"
						"\"123\"}],\"logic\":\"and\",\"effect\":\"allow\"}";
...
static const char* test_permission_func_basic() {
	...
	user_t user;
	user_create(umgr, "dave", "p", "d@e.c", "D", &user);

	policy_t	pol;
	const char* rules = "{\"functions\":[{\"code\":\"user:create\",\"effect\":\"allow\"},{\"code\":\"user:delete\","
...
static const char* test_permission_lbac_basic() {
	...
	user_t user;
	user_create(umgr, "eve", "p", "e@e.c", "E", &user);
...
static const char* test_permission_location_basic() {
	...
	user_t user;
	user_create(umgr, "frank", "p", "f@e.c", "F", &user);
...
static const char* test_permission_deny_override() {
	...
	user_t user;
	user_create(umgr, "grace", "p", "g@e.c", "G", &user);
...
static const char* test_permission_disabled_policy() {
	...
	user_t user;
	user_create(umgr, "heidi", "p", "h@e.c", "H", &user);
...
static const char* test_permission_no_match() {
	...
	user_t user;
	user_create(umgr, "ivan", "p", "i@e.c", "I", &user);
